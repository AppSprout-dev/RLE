...
``felix-agent-sdk`` is not modified by RLE harness-layer knobs. Roster
ablation (``roles`` / ``exclude_agent``), per-role ``role_timeout_s``, and
ActionResolver merge policy live in this package / ``rle.orchestration``.
See ``docs/harness-felix.md``.
"""
