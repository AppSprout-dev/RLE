...
`scripts/check_harness_boundary.py` (CI) rejects third-party harness *product* names and
stray `felix_agent_sdk` imports in this tree.
...
- Optional: wandb, huggingface-hub (`uv sync --extra tracking`). W&B stays opt-in (`--wandb` on `run_benchmark.py` only); see [README — Weights and Biases (opt-in)](README.md#weights-and-biases-opt-in).
