...
- Optional: wandb, huggingface-hub (`uv sync --extra tracking`). W&B stays opt-in (`--wandb` on `run_benchmark.py` only); see [README — Weights & Biases (opt-in)](README.md#weights--biases-opt-in).
